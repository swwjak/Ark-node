#!/bin/bash
# restore-test-v2.sh — Fixed recovery proof test
# Fixes: loop device cleanup, phone path
...
echo "========================================="
echo "  RECOVERY PROOF TEST v2"
echo "  $(date)"
echo "========================================="

# Cleanup any old loop devices
...
echo ""
echo "--- [4] Mount Test ---"
MOUNT="/tmp/recovery-proof"
rm -rf "$MOUNT"
mkdir -p "$MOUNT"

for img in Adam.img Eve.img; do
    LOOP=$(sudo losetup --find --show --partscan "$TREE/Images/$img")
    sleep 2
    ...
    if sudo mount "${LOOP}p2" "$MOUNT_POINT" 2>/dev/null; then
        ...
        if [ "$img" = "Adam.img" ]; then
            ...
            [ -f "$MOUNT_POINT/boot/first-boot" ] && check "Adam boot marker" ok || check "Adam boot marker" fail
            [ -f "$MOUNT_POINT/boot/ssh" ] && check "Adam SSH" ok || check "Adam SSH" fail
            [ "$(cat $MOUNT_POINT/etc/hostname 2>/dev/null)" = "adam" ] && check "Adam hostname" ok || check "Adam hostname" fail
        fi
        
        if [ "$img" = "Eve.img" ]; then
            [ -f "$MOUNT_POINT/home/pi/first-boot.sh" ] && check "Eve first-boot.sh" ok || check "Eve first-boot.sh" fail
            [ -f "$MOUNT_POINT/boot/first-boot" ] && check "Eve boot marker" ok || check "Eve boot marker" fail
            [ -f "$MOUNT_POINT/boot/ssh" ] && check "Eve SSH" ok || check "Eve SSH" fail
            [ "$(cat $MOUNT_POINT/etc/hostname 2>/dev/null)" = "eve" ] && check "Eve hostname" ok || check "Eve hostname" fail
...
echo ""
echo "--- [7] Phone Backup ---"
PHONE="/data/data/com.termux/files/home/ARCHIVE"
[ -d "$PHONE" ] && check "Phone archive" ok || check "Phone archive" fail
[ -f "$PHONE/KNOTS.md" ] && check "Phone KNOTS.md" ok || check "Phone KNOTS.md" fail
[ -f "$PHONE/RECOVERY.md" ] && check "Phone RECOVERY.md" ok || check "Phone RECOVERY.md" fail
[ -f "$PHONE/simulization-factory/Adam.img.gz" ] && check "Phone Adam.img.gz" ok || check "Phone Adam.img.gz" fail
[ -f "$PHONE/simulization-factory/Eve.img.gz" ] && check "Phone Eve.img.gz" ok || check "Phone Eve.img.gz" fail
...
if [ "$FAIL" -eq 0 ]; then
    ...
    echo ""
    echo "  GAPS REMAIN: $FAIL items need attention."
    exit 1
fi